...
export interface Spec extends TurboModule {
  ...
  // ── Screen / Utility ─────────────────────────────────────────────────
  /**
   * Controls the iOS idle timer (screen auto-lock).
   * Call with enabled=true to keep screen awake during long benchmarks.
   * This directly sets UIApplication.isIdleTimerDisabled on the main thread.
   */
  setKeepScreenAwake(args: {
    enabled: boolean;
  }): Promise<{ keepAwake: boolean }>;
}

export default TurboModuleRegistry.get<Spec>('CblSwift');
